...
public class RequestTokenParam {
	...
	public void validateRequestToken(String consumerSecret, String callbackURL) throws Exception {
...
//		valid = sha1.isValid(OAuthUtil.decode(this.signature), this.baseString);
		...
		if (!valid) 
			throw new OAuthException("Invalid signature", "E001-1");
		...
		if (timeSpan < 0 || timeSpan / 60 > 60 ) 
			throw new OAuthException("Invalid timestamp", "E001-2");
		
		System.out.println("### timsSpan : " + timeSpan );
		
		System.out.println("###[callback] Client callbackURL : " + this.callback);
		System.out.println("###[callback] Consumer callbackURL : " + OAuthUtil.encode(callbackURL));
		OAuthUtil.decode(callbackURL).equals(this.callback);
		if( !this.callback.equals(OAuthUtil.encode(callbackURL))) {
			throw new OAuthException("Mismatch callbackURL", "E001-3");
		}
	}
