...
public class AuthorizationServerConfig {
	...
	TokenSettings tokenSettings() {
		return TokenSettings.builder().accessTokenTimeToLive(Duration.ofMinutes(60))
				.refreshTokenTimeToLive(Duration.ofDays(30)).idTokenSignatureAlgorithm(SignatureAlgorithm.RS256)
	...
	RegisteredClientRepository registeredClientRepository(JdbcTemplate jdbcTemplate) {
		...
		if (client1 == null) {
			RegisteredClient registeredClient = RegisteredClient.withId(UUID.randomUUID().toString())
					...
					.redirectUri("https://oidcdebugger.com/debug").redirectUri("http://jcornor.com:8000/callback")
					.redirectUri("https://oauth.pstmn.io/v1/callback")
					.redirectUri("http://server:8000/swagger-ui/oauth2-redirect.html").scope(OidcScopes.OPENID)
					.scope("profiles").scope("contacts").scope("messages").tokenSettings(tokenSettings()).build();
